...
**Tech Stack:** Node 22, TypeScript (ESM), NestJS 11, Postgres 17, Drizzle ORM + `postgres.js`, global `fetch`, `cheerio`, `@anthropic-ai/sdk`, `@nestjs/schedule`, `zod`, Vitest (+ `unplugin-swc`), Docker Compose (backend only), Vite + React for the dashboard.
...
- **Vitest is the test runner in both projects.** The backend needs `unplugin-swc`: Nest resolves constructor dependencies from `emitDecoratorMetadata`, which Vitest's default esbuild transform does not emit, and DI then fails at runtime in tests only.
- Every backend test file relies on `reflect-metadata` being imported once, from `tests/setup.ts`.
...
export default defineConfig({
  ...
  // Required: Nest resolves constructor dependencies from
  // emitDecoratorMetadata, which Vitest's default esbuild transform does not
  // emit. Without this, DI fails with "Nest can't resolve dependencies" in
  // tests only — the compiled build works fine, which makes it baffling.
  plugins: [swc.vite({ module: { type: 'es6' } })],
});
```

Also create `backend/tests/setup.ts`:
...
- Create: `backend/tests/fixtures/greenhouse-acme.json`, `backend/tests/fixtures/lever-acme.json`, `backend/tests/fixtures/ashby-acme.json`
...
- Test: `backend/tests/classifier/prompt.test.ts`
...
import { describe, it, expect } from 'vitest';
import { buildPrompt } from '../../src/classifier/prompt.js';
import type { RawPosting } from '../../src/types.js';
...
Run: `npm test -- tests/classifier/prompt.test.ts`
...
- Test: `backend/tests/db/postings.repository.test.ts`
...
Needs a live Postgres; skips cleanly without `DATABASE_URL_TEST` so the default suite stays offline.

```ts
// backend/tests/db/postings.repository.test.ts
import { describe, it, expect, beforeAll } from 'vitest';
import { Test } from '@nestjs/testing';
import { DatabaseModule, DB } from '../../src/db/db.module.js';
import { PostingsRepository } from '../../src/db/postings.repository.js';
...
describe.skipIf(!url)('PostingsRepository', () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [PostingsRepository, { provide: DB, useValue: db }],
    }).compile();

    repo = moduleRef.get(PostingsRepository);
  });
...
Run: `DATABASE_URL_TEST=postgres://jobradar:jobradar@localhost:5432/jobradar npm test -- tests/db/postings.repository.test.ts`
...
import type { FitVerdict, RawPosting, Verdict } from '../types.js';
...
- [ ] **Step 7: Confirm the suite still skips cleanly without a database**

Run: `npm test`
Expected: repository tests skipped; every other suite passes.
...
- Test: `backend/tests/notify/telegram.notifier.test.ts`
...
```ts
// backend/tests/notify/telegram.notifier.test.ts
import { describe, it, expect } from 'vitest';
import { TelegramNotifier } from '../../src/notify/telegram.notifier.js';
import type { PendingNotification } from '../../src/db/postings.repository.js';
...
Run: `npm test -- tests/notify/telegram.notifier.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Create `backend/src/notify/types.ts`**

```ts
import type { PendingNotification } from '../db/postings.repository.js';
...
import type { PendingNotification } from '../db/postings.repository.js';
import type { Notifier, TelegramOptions } from './types.js';
...
import { NOTIFIER, type Notifier } from './types.js';
...
- Test: `backend/tests/classifier/classifier.service.test.ts`, `backend/tests/sources/sources.factory.test.ts`
...
```ts
// backend/tests/sources/sources.factory.test.ts
import { describe, it, expect } from 'vitest';
import { buildSources } from '../../src/sources/sources.factory.js';
...
```ts
// backend/tests/classifier/classifier.service.test.ts
import { describe, it, expect } from 'vitest';
import { Test } from '@nestjs/testing';
import { ClassifierService } from '../../src/classifier/classifier.service.js';
import { LLM_PROVIDER } from '../../src/classifier/classifier.module.js';
...
Run: `npm test -- tests/sources/sources.factory.test.ts tests/classifier/classifier.service.test.ts`
...
import type { AppConfig, Profile, Rubric, SourcesConfig } from './schema.js';
...
import { createAtsSource } from './ats.js';
import { createDjinniSource } from './djinni.js';
import { createDouSource } from './dou.js';
...
import { buildSources } from './sources.factory.js';
import type { JobSource } from '../types.js';
...
import { Inject, Injectable } from '@nestjs/common';
import { AppConfigService } from '../config/app-config.service.js';
import { LLM_PROVIDER } from './classifier.module.js';
...
Append to `backend/tests/classifier/classifier.service.test.ts`:

```ts
import { selectProvider } from '../../src/classifier/classifier.module.js';
...
- Test: `backend/tests/pipeline/pipeline.service.test.ts`
...
```ts
// backend/tests/pipeline/pipeline.service.test.ts
...
import { ClassifierService } from '../../src/classifier/classifier.service.js';
import { AppConfigService } from '../../src/config/app-config.service.js';
import { SOURCES } from '../../src/sources/sources.module.js';
import { NOTIFIER } from '../../src/notify/types.js';
...
function fakeRepo() {
  ...
  return {
    ...
    upsert: vi.fn(async (p: RawPosting) => {
      const isNew = !seen.has(p.id); seen.add(p.id); return { isNew };
    }),
    hasScore: vi.fn(async (id: string) => withScore.has(id)),
...
Run: `npm test -- tests/pipeline/pipeline.service.test.ts`
Expected: FAIL — module not found.
...
import { PostingsRepository } from '../db/postings.repository.js';
import { ClassifierService } from '../classifier/classifier.service.js';
import { AppConfigService } from '../config/app-config.service.js';
import { SOURCES } from '../sources/sources.module.js';
import { NOTIFIER, type Notifier } from '../notify/types.js';
...
```ts
import { LLM_PROVIDER } from '../classifier/classifier.module.js';
...
- Test: `backend/tests/worker/worker.service.test.ts`
...
```ts
// backend/tests/worker/worker.service.test.ts
import { describe, it, expect, vi } from 'vitest';
import { Test } from '@nestjs/testing';
...
describe('WorkerService', () => {
  ...
  it('swallows a thrown pipeline so the scheduler survives', async () => {
    const run = vi.fn(async () => { throw new Error('exploded'); });
  ...
  it('skips a tick while a previous run is still active', async () => {
    ...
    const run = vi.fn(async () => { await gate; return summary; });
    const svc = await build(run);
...
Run: `npm test -- tests/worker/worker.service.test.ts`
Expected: FAIL — module not found.
...
- Test: `backend/tests/api/postings.controller.test.ts`
...
```ts
// backend/tests/api/postings.controller.test.ts
import { describe, it, expect, beforeAll } from 'vitest';
import { Test } from '@nestjs/testing';
import request from 'supertest';
import type { INestApplication } from '@nestjs/common';
import { PostingsController } from '../../src/api/postings.controller.js';
import { HealthController } from '../../src/api/health.controller.js';
import { DashboardQueries } from '../../src/api/dashboard.queries.js';
import type { HealthRow, PostingRow } from '../../src/api/api.schema.js';
...
Run: `npm test -- tests/api/postings.controller.test.ts`
Expected: FAIL — modules not found.
...
import type { HealthRow, PostingFilters, PostingRow } from './api.schema.js';
...
import { PostingFiltersSchema, type PostingFilters } from './api.schema.js';
...
- [ ] `cd backend && npm test` — green with no database present (repository tests skip)
- [ ] `cd backend && DATABASE_URL_TEST=postgres://jobradar:jobradar@localhost:5432/jobradar npm test` — repository tests green too
