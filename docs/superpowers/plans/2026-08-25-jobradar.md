...
- [x] **Step 1: Create `backend/package.json`**
...
- [x] **Step 2b: Create `backend/tsconfig.build.json`**
...
- [x] **Step 3: Create `backend/vitest.config.ts` (integration suite only)**
...
- [x] **Step 5: Create `src/types.ts` (the subset needed now)**
...
- [x] **Step 6: Create `src/db/client.ts`**
...
- [x] **Step 7: Create `drizzle.config.ts`**
...
- [x] **Step 8: Create `docker-compose.yml` with `db` and `migrate` only**
...
- [x] **Step 9: Create `Dockerfile` at the repository root**
...
- [x] **Step 10: Create `.env.example` and `.dockerignore`**
...
- [x] **Step 13: Run the test**

Run: `npm test -- src/db/schema.spec.ts`
Expected: PASS

- [x] **Step 14: Verify migrations apply against a real database**
...
- [x] **Step 3: Create `src/config/schema.ts`**
...
- [x] **Step 4: Create `src/config/load.ts`**
...
- [x] **Step 6: Create the shipped example config**
...
- [x] **Step 1: Append to `src/types.ts`**
...
- [x] **Step 2: Create the Greenhouse fixture**
...
- [x] **Step 5: Create `src/sources/ats.ts`**
...
- [x] **Step 7: Add Lever and Ashby cases to the test file**
...
- [x] **Step 1: Capture a real listing page as a fixture**
...
- [x] **Step 4: Create `src/sources/djinni.ts`**
...
- [x] **Step 4: Create `src/sources/dou.ts`**
...
- [x] **Step 3: Create `src/filters.ts`**
...
- [x] **Step 3: Create `src/classifier/rubric.ts`**
...
- [x] **Step 1: Append `FitVerdict` to `src/types.ts`**
...
- [x] **Step 4: Create `src/classifier/providers/types.ts`**
...
- [x] **Step 5: Create `src/classifier/providers/fake.ts`**
...
- [x] **Step 6: Create `src/classifier/schema.ts`**
...
- [x] **Step 7: Create `src/classifier/prompt.ts`**
...
- [x] **Step 8: Run the classifier building-block tests**
...
- [x] **Step 3: Create `src/classifier/providers/anthropic.ts`**
...
- [x] **Step 3: Create `src/classifier/providers/openai-compat.ts`**
...
- [x] **Step 3: Create `backend/src/db/db.module.ts`**
...
- [x] **Step 4: Replace the shutdown hook with an injected connection**
...
- [x] **Step 5: Create `backend/src/db/postings.repository.ts`**
...
- [x] **Step 7: Confirm the unit suite is unaffected**
...
- [x] **Step 3: Create `backend/src/notify/types.ts`**
...
- [x] **Step 4: Create `backend/src/notify/telegram.notifier.ts`**
...
- [x] **Step 5: Create `backend/src/notify/notify.module.ts`**
...
- [x] **Step 1: Write the failing sources test**
...
- [x] **Step 2: Write the failing classifier-service test**
...
- [x] **Step 4: Create `backend/src/config/app-config.service.ts`**
...
- [x] **Step 5: Create `backend/src/config/config.module.ts`**
...
- [x] **Step 6: Create `backend/src/sources/sources.factory.ts` and `sources.module.ts`**
...
- [x] **Step 7: Create `backend/src/classifier/classifier.service.ts`**
...
- [x] **Step 8: Create `backend/src/classifier/classifier.module.ts`**
...
- [x] **Step 9: Add the provider-selection test**
...
- [x] **Step 3: Create `backend/src/pipeline/pipeline.service.ts`**
...
- [x] **Step 4: Use the active provider's threshold**
...
- [x] **Step 5: Create `backend/src/pipeline/pipeline.module.ts`**
...
- [x] **Step 3: Create `backend/src/worker/worker.service.ts`**
...
- [x] **Step 4: Create `backend/src/worker/worker.module.ts` and `backend/src/app.module.ts`**
...
- [x] **Step 5: Create `backend/src/worker.main.ts` and `backend/src/once.ts`**
...
- [x] **Step 7: Add the `worker` service to `docker-compose.yml`**
...
- [x] **Step 8: Verify the worker boots, runs, and exits cleanly**
...
- [x] **Step 3: Create `backend/src/api/api.schema.ts`**
...
- [x] **Step 4: Create `backend/src/api/zod-validation.pipe.ts`**
...
- [x] **Step 5: Create `backend/src/api/dashboard.queries.ts`**
...
- [x] **Step 6: Create the controllers and `api.module.ts`**
...
- [x] **Step 7: Create `backend/src/main.ts`**
...
- [x] **Step 9: Add the `api` service to `docker-compose.yml`**
...
- [x] **Step 10: Verify against a real database**
...
- [x] **Step 2: Create `dashboard/package.json` scripts block**
...
- [x] **Step 3: Create `dashboard/tsconfig.json`**
...
- [x] **Step 4: Create `dashboard/vite.config.ts`**
...
- [x] **Step 5: Create `dashboard/tests/setup.ts` and `dashboard/index.html`**
...
- [x] **Step 8: Create `dashboard/src/api/types.ts`**
...
- [x] **Step 9: Create `dashboard/src/api/client.ts`**
...
- [x] **Step 11: Write the failing hook test**
...
- [x] **Step 13: Create `dashboard/src/hooks/useApi.ts`**
...
- [x] **Step 15: Create `dashboard/src/styles.css`**
...
- [x] **Step 3: Create `dashboard/src/components/VerdictBadge.tsx`**
...
- [x] **Step 4: Create `dashboard/src/components/PostingsTable.tsx`**
...
- [x] **Step 6: Create `dashboard/src/components/Filters.tsx`**
...
- [x] **Step 7: Create `dashboard/src/components/SourceHealth.tsx`**
...
- [x] **Step 8: Write the failing App test**
...
- [x] **Step 10: Create `dashboard/src/App.tsx`**
...
- [x] **Step 11: Create `dashboard/src/main.tsx`**
...
> **Superseded — implemented, then intentionally removed.** This task shipped in
> `49aee93` and was undone in `bede068` ("drop static serving, enable CORS
> instead") as part of the per-project-docker-config plan. The dashboard deploys
> to its own server, so same-origin static serving could never fire:
> `ServeStaticModule`, `@nestjs/serve-static`, `STATIC_ROOT`, and the
> `./dashboard/dist:/app/public` mount are all gone. The SPA now reaches the API
> cross-origin via `CORS_ORIGIN` (`backend/src/cors.ts`,
> `backend/src/main.ts`). The steps below are checked because they were
> executed; do not expect their result in the current tree.
...
- [x] **Step 1: Build the dashboard on the host**
...
- [x] **Step 2: Mount the build into the `api` service**
...
- [x] **Step 3: Verify the SPA is served same-origin**
...
- [x] **Step 4: Add the backend dev profile**
...
- [x] **Step 5: Verify the development loop**
...
- [x] **Step 2: Create the opt-in integration test**
...
- [x] **Step 3: Verify everything from a clean state**
...
> **Left unchecked deliberately.** These were not replayed when the plan's step
> boxes were back-filled, and two are now stale: the API serves no static
> assets, so `http://localhost:8080` no longer renders the dashboard (open the
> Vite dev server or the dashboard's own deployment instead), and the Docker
> commands run from `backend/`, not the repository root.
