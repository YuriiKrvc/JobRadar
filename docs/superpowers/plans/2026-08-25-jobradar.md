...
**Tech Stack:** Node 22, TypeScript (ESM), Postgres 17, Drizzle ORM + `postgres.js`, global `fetch`, `cheerio`, `@anthropic-ai/sdk`, `hono`, `node-cron`, `zod`, `vitest`, Docker Compose.
...
Deferred from Task 16 because it imports `App`, which only exists now.
